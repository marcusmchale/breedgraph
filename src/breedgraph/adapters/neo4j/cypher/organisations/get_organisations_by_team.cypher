...
OPTIONAL MATCH (captured)<-[:INCLUDES*0..]-(root:Team)
WHERE NOT (root)<-[:INCLUDES]-(:Team)  // root has no incoming INCLUDES

// Now get the complete organization starting from each root
OPTIONAL MATCH (root)-[:INCLUDES*1..]->(member:Team)
...
RETURN [ team IN organisation |
  team {
    ...
    includes: [(team)-[includes:INCLUDES]->(child:Team) | [team.id, child.id, {label:type(includes)}]]
  }
] AS organisation