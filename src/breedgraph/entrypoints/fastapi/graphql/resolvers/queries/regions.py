...
location_template = ObjectType("LocationTemplate")
graphql_resolvers.register_type_resolvers(location, location_template)
...
@graphql_payload
@require_authentication
async def get_locations(_, info, location_ids: List[int]|None = None) -> List[LocationOutput]:
...
@graphql_query.field("regionsLocationsByType")
@graphql_payload
@require_authentication
async def get_locations_by_type(_, info, location_type_id: int | None = None) -> List[LocationOutput]:
...
async def resolve_region(obj, info) -> LocationOutput:
    await update_locations_map(info.context, location_ids=[obj.region])
    return info.context.get('locations_map').get(obj.region)
...
@location_template.field("typeId")
async def resolve_type(obj, _):
    return obj.type
