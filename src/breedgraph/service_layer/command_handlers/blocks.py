...
async def update_unit(
    ...
    async with uow.get_repositories(user_id=user_id, release=release) as uow:
        ...
        if cmd.children is not None:
            ...
            for child_id in cmd.children:
                if child_id in block.entries:
                    ...
                    child_block = await uow.blocks.get(unit_id=child_id)
...
async def delete_unit(
    ...
    async with uow.get_repositories(user_id=cmd.user) as uow:
        block = await uow.blocks.get(unit_id=cmd.unit_id)
