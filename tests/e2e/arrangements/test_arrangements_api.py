...
from tests.e2e.regions.post_methods import post_to_locations_by_type
...
async def test_create_layout(
    ...
    locations_response = await post_to_locations_by_type(client=client, token=first_user_login_token, location_type_id = field_type_id)
    locations_payload = get_verified_payload(locations_response, "regionsLocationsByType")
    ...
    layout_request_response = await post_to_layouts(client, layout_ids=[layout_id], token=first_user_login_token)
    ...
    assert layout_payload.get('result')[0].get('type').get('id') == indexed_rows_type_id
...
async def test_extended_layout(
    ...
    lab_location_response = await post_to_locations_by_type(
        client=client, token=first_user_login_token, location_type_id = lab_type_id
    )
    lab_location_payload = get_verified_payload(lab_location_response, "regionsLocationsByType")
    ...
    facility_layout_request_response = await post_to_layouts(client, layout_ids=[facility_layout_id], token=first_user_login_token)
    ...
    chamber_layout_request_response = await post_to_layouts(client, layout_ids=[chamber_layout_id], token=first_user_login_token)
