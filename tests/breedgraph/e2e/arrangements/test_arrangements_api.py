...
from tests.breedgraph.e2e.arrangements.post_methods import post_to_create_layout, post_to_arrangements, post_to_layouts
...
async def test_create_layout(
    ...
    assert arrangements_at_location_payload.get('result')[0].get('type').get('id') == str(type_id)
    ...
    assert layout_payload.get('result')[0].get('type').get('id') == str(type_id)
