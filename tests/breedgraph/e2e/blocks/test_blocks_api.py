...
from tests.breedgraph.e2e.blocks.post_methods import post_to_create_unit, post_to_blocks, post_to_units, post_to_add_position
...
async def test_add_unit_position(
    ...
    while True:
        ...
        if tree_unit.get('subject').get('id') == str(tree_type):
            break
        else:
