...
@pytest.mark.asyncio(scope="session")
async def test_edit_reference(neo4j_tx, stored_account, stored_organisation, lorem_text_generator):
    repo = Neo4jReferencesRepository(
        neo4j_tx,
        user_id=stored_account.user.id,
        read_teams={stored_organisation.root.id},
...
@pytest.mark.asyncio(scope="session")
async def test_file_reference(neo4j_tx, stored_account, stored_organisation, lorem_text_generator):
    repo = Neo4jReferencesRepository(
        neo4j_tx,
        user_id=stored_account.user.id,
        read_teams={stored_organisation.root.id},
