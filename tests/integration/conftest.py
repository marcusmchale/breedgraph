...
async def neo4j_access_control_service(uncommitted_neo4j_tx) -> AsyncGenerator[Neo4jAccessControlService, None]:
    """Test controllers service fixture for integration tests"""
    yield Neo4jAccessControlService(tx=uncommitted_neo4j_tx)
...
@pytest_asyncio.fixture(scope='session')
async def organisations_repo(uncommitted_neo4j_tx, stored_account) -> AsyncGenerator[Neo4jOrganisationsRepository, None]:
    yield Neo4jOrganisationsRepository(uncommitted_neo4j_tx, user_id=stored_account.user.id)
...
@pytest_asyncio.fixture(scope='session')
async def ontology_service(ontology_persistence_service, stored_account) -> AsyncGenerator[OntologyApplicationService, None]:
    yield OntologyApplicationService(
        ontology_persistence_service,
        user_id=stored_account.user.id,
        role=stored_account.user.ontology_role
    )
...
@pytest_asyncio.fixture(scope='session')
async def regions_repo(uncommitted_neo4j_tx, stored_account, stored_organisation, neo4j_access_control_service) -> AsyncGenerator[Neo4jRegionsRepository, None]:
...
@pytest_asyncio.fixture(scope='session')
async def germplasm_persistence_service(uncommitted_neo4j_tx, stored_account, stored_organisation, neo4j_access_control_service, ) -> AsyncGenerator[Neo4jGermplasmPersistenceService, None]:
...
async def tree_subject(ontology_service) -> AsyncGenerator[SubjectStored, None]:
    if await ontology_service.get_entry(name="Tree", label="Subject") is None:
    ...
    subject = await ontology_service.get_entry(name="Tree", label="Subject")
    yield subject

@pytest_asyncio.fixture(scope='session')
async def state_type(ontology_service, country_type) -> AsyncGenerator[LocationTypeStored, None]:
    state_type = await ontology_service.get_entry(name="State", label="LocationType")
...
async def field_type(ontology_service) -> AsyncGenerator[LocationTypeStored, None]:
    if await ontology_service.get_entry(name="Field", label="LocationType") is None:
    ...
    field_type = await ontology_service.get_entry(name="Field", label="LocationType")
    yield field_type

@pytest_asyncio.fixture(scope='session')
async def row_layout_type(ontology_service) -> AsyncGenerator[LayoutTypeStored, None]:
    if await ontology_service.get_entry(name="Rows", label="LayoutType") is None:
    ...
    row_layout_type = await ontology_service.get_entry(name="Rows", label="LayoutType")
    yield row_layout_type

@pytest_asyncio.fixture(scope='session')
async def grid_layout_type(ontology_service) -> AsyncGenerator[LayoutTypeStored, None]:
    if await ontology_service.get_entry(name="grid", label="LayoutType") is None:
    ...
    grid_layout_type = await ontology_service.get_entry(name="grid", label="LayoutType")
...
async def height_trait(uncommitted_neo4j_tx, ontology_service, tree_subject) -> AsyncGenerator[TraitStored, None]:
    if await ontology_service.get_entry(name="Height", label="Trait") is None:
    ...
    height_trait = await ontology_service.get_entry(name="Height", label="Trait")
    yield height_trait

@pytest_asyncio.fixture(scope='session')
async def cm_scale(ontology_service) -> AsyncGenerator[ScaleStored, None]:
    if await ontology_service.get_entry(name="cm", label="Scale") is None:
    ...
    scale = await ontology_service.get_entry(name="cm", label="Scale")
    yield scale

@pytest_asyncio.fixture(scope='session')
async def tape_method(ontology_service) -> AsyncGenerator[ObservationMethodStored, None]:
    if await ontology_service.get_entry(name="tape", label="ObservationMethod") is None:
    ...
    method = await ontology_service.get_entry(name="tape", label="ObservationMethod")
    yield method

@pytest_asyncio.fixture(scope='session')
async def height_variable(ontology_service, height_trait, cm_scale, tape_method) -> AsyncGenerator[VariableStored, None]:
    if await ontology_service.get_entry(name="Height", label="Variable") is None:
    ...
    variable = await ontology_service.get_entry(name="Height", label="Variable")
    yield variable
...
@pytest_asyncio.fixture(scope='session')
async def arrangements_repo(uncommitted_neo4j_tx, stored_account, stored_organisation, neo4j_access_control_service) -> AsyncGenerator[Neo4jArrangementsRepository, None]:
...
@pytest_asyncio.fixture(scope='session')
async def crossing_method(ontology_service) -> AsyncGenerator[GermplasmMethodStored, None]:
    if await ontology_service.get_entry(name="Crossing", label="GermplasmMethod") is None:
        crossing_method = GermplasmMethodInput(
             name='Crossing',
    ...
    crossing_method = await ontology_service.get_entry(name="Crossing", label="GermplasmMethod")
    yield crossing_method

@pytest_asyncio.fixture(scope='session')
async def references_repo(uncommitted_neo4j_tx, stored_account, stored_organisation, neo4j_access_control_service) -> AsyncGenerator[Neo4jReferencesRepository, None]:
...
@pytest_asyncio.fixture(scope='session')
async def blocks_repo(uncommitted_neo4j_tx, stored_account, stored_organisation, neo4j_access_control_service) -> AsyncGenerator[Neo4jBlocksRepository, None]:
...
@pytest_asyncio.fixture(scope='session')
async def programs_repo(uncommitted_neo4j_tx, stored_account, stored_organisation, neo4j_access_control_service) -> AsyncGenerator[Neo4jProgramsRepository, None]:
...
@pytest_asyncio.fixture(scope='session')
async def people_repo(uncommitted_neo4j_tx, stored_account, stored_organisation, neo4j_access_control_service) -> AsyncGenerator[Neo4jPeopleRepository, None]:
...
@pytest_asyncio.fixture(scope='session')
async def datasets_repo(uncommitted_neo4j_tx, neo4j_access_control_service, stored_account, stored_organisation) -> AsyncGenerator[Neo4jDatasetsRepository, None]:
...
async def stored_dataset(datasets_repo, tree_block, height_variable, stored_person, example_external_reference) -> AsyncGenerator[DataSetStored, None]:
    dataset_input = DataSetInput(
        ontology_id=height_variable.id,
