...
async def test_add_layout_command_simple(
    ...
    async with bus.uow.get_repositories(user_id=first_account_with_all_affiliations.user.id) as uow:
        async for arrangement in uow.arrangements.get_all(location_id = location.id):
